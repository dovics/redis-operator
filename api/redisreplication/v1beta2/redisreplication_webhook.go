...
var _ admission.CustomValidator = &RedisReplication{}

// ValidateCreate implements admission.CustomValidator so a webhook will be registered for the type
func (r *RedisReplication) ValidateCreate(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	replication, ok := obj.(*RedisReplication)
	if !ok {
		return nil, apierrors.NewBadRequest(fmt.Sprintf("expected a RedisReplication object but got %T", obj))
	}
...
func (r *RedisReplication) ValidateUpdate(ctx context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	oldReplication, ok := oldObj.(*RedisReplication)
	if !ok {
		return nil, apierrors.NewBadRequest(fmt.Sprintf("expected a RedisReplication object but got %T", oldObj))
	}
	newReplication, ok := newObj.(*RedisReplication)
	if !ok {
		return nil, apierrors.NewBadRequest(fmt.Sprintf("expected a RedisReplication object but got %T", newObj))
	}
...
func (r *RedisReplication) ValidateDelete(ctx context.Context, obj runtime.Object) (admission.Warnings, error) {
	replication, ok := obj.(*RedisReplication)
	if !ok {
		return nil, apierrors.NewBadRequest(fmt.Sprintf("expected a RedisReplication object but got %T", obj))
	}
	redisreplicationlog.Info("validate delete", "name", replication.Name)
